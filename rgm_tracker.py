...
PHASES = [
    ...
    # "Bracket Play" is the knockout half of a tournament, whatever it is called
    # -- the Euro Championship for Small Countries uses it where others say
    # Quarterfinals.
    ("Bracket Play", "playoffs"), ("Knockout", "playoffs"),
...
# Competitions RealGM does not name, identified by who is playing.
#
# Some national box scores carry no competition line at all: San Marino at Malta
# on 27 June 2026 has a date, a score and "Attendance", and nothing else. Those
# games fall back to the section default and every one of them reads "National
# Teams", which is no label.
#
# The teams identify them well enough. One rule per line:
#
#     San Marino, Malta, Andorra, Gibraltar = Euro Champ for Small Countries
#
# A rule fires only when EITHER side is listed AND the page named no competition,
# so it can never override something RealGM actually said.
UNKNOWN_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                            "nba_rgm_gr", "unnamed_competitions.txt")
UNKNOWN_TEMPLATE = """# Competitions RealGM leaves unlabelled, identified by team.
# TEAM, TEAM, TEAM = Competition name
# Only applied when the box score names no competition of its own.
San Marino, Malta, Andorra, Gibraltar, Monaco, Liechtenstein = Euro Champ for Small Countries
"""
_unknown_cache = [None]


def load_unknown():
    if _unknown_cache[0] is not None:
        return _unknown_cache[0]
    rules = []
    if not os.path.exists(UNKNOWN_FILE):
        try:
            os.makedirs(os.path.dirname(UNKNOWN_FILE), exist_ok=True)
            with open(UNKNOWN_FILE, "w", encoding="utf-8") as f:
                f.write(UNKNOWN_TEMPLATE)
        except OSError:
            pass
    try:
        with open(UNKNOWN_FILE, encoding="utf-8") as f:
            for line in f:
                ...
                if "=" in t:
                    teams, name = t.split("=", 1)
                    names = [x.strip().lower() for x in teams.split(",") if x.strip()]
                    if names and name.strip():
                        rules.append((names, name.strip()))
    except OSError:
        pass
    _unknown_cache[0] = rules
    return rules


def competition_from_teams(a, b):
    ta, tb = (a or "").strip().lower(), (b or "").strip().lower()
    for names, comp in load_unknown():
        if ta in names or tb in names:
            return comp
    return ""
...
def competition(html):
    ...
    # STRUCTURE FIRST: the competition is a link to /national/tournament/...
    #
    #   June 27, 2026
    #   Euro Champ for Small Countries - Bracket Play
    #   Tercentenary Sports Hall (Gibraltar, Gilbraltar)
    #   Attendance: N/A
    #
    # Reading the flattened text from the date to "Attendance" worked until a
    # game had a VENUE line, which pushed the match past its length limit and
    # left every Euro Championship for Small Countries game unlabelled. The
    # tournament link is unambiguous where the text is not -- but the site
    # navigation is full of the same links, so only one AFTER the date counts.
    date_m = re.search(
        r"(?:January|February|March|April|May|June|July|August|September"
        r"|October|November|December)\s+\d{1,2},\s+\d{4}", html[:60000], re.I)
    if date_m:
        after = html[date_m.end():date_m.end() + 4000]
        a = re.search(r'<a[^>]+href="/national/tournament/[^"]*"[^>]*>(.*?)</a>'
                      r'(.{0,60}?)(?:<a|<br|</p|</div|Attendance)', after, re.S | re.I)
        if a:
            comp = " ".join(re.sub(r"<[^>]+>", " ", a.group(1)).split())
            tail = " ".join(re.sub(r"<[^>]+>", " ", a.group(2)).split())
            stage = tail.lstrip(" -\u2013\u2014").strip() if tail.strip(" -\u2013\u2014") else ""
            if comp:
                return comp, stage

    # Falling back to the text form, for pages with no tournament link.
...
def parse_box(html, slug, section="international"):
    ...
    # A competition the page did not name, recovered from who is playing. Only
    # when the page said nothing -- a rule must never override RealGM.
    if len(teams) == 2 and cfg.get("league") and league_name == cfg["league"]:
        guess = competition_from_teams(teams[0]["team"], teams[1]["team"])
        if guess:
            league_name = guess
